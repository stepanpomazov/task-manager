...
export async function renderHeader(containerId = "header-container") {
  ...
  const showButton = createShowButton(
    monthGroup.monthSelect,
    yearGroup.yearInput
  );

  const headerLeft = document.createElement("div");
  headerLeft.className = "header-left";
  headerLeft.appendChild(monthGroup.monthGroup);
  headerLeft.appendChild(yearGroup.yearGroup);
  headerLeft.appendChild(departmentGroup);
  ...
  menuContainer.appendChild(headerLeft);
  menuContainer.appendChild(headerRight);
  ...
  return {
    monthSelect: monthGroup.monthSelect,
    yearInput: yearGroup.yearInput,
  };
}
    ...
    const renderDepartment = (items, level = 0) => {
      ...
      items.forEach((item) => {
        ...
        deptContent.addEventListener("click", (e) => {
          e.stopPropagation();
          deptTrigger.querySelector(".dept-trigger-text").textContent =
            item.name;
